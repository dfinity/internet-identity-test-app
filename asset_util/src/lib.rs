//! Asset certification utility library for asset serving canisters.
//!
//! This is a copy of Internet Identity's `asset_util` crate, which lives at
//! `src/asset_util` in <https://github.com/dfinity/internet-identity>. The copy
//! exists so this repository builds without a path dependency on the II tree;
//! `HeaderField` is declared here instead of being imported from
...
/// An HTTP header, as used by the IC HTTP gateway protocol.
